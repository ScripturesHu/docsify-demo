...
数据结构是指所有数据元素以及数据元素之间的关系，可以看作是相互之间存在着某种特定关系的数据元素的集合，即可把数据结构看成是带结构的数据元素的集合。数据结构包括如下三个方面：

- 数据元素之间的逻辑关系，即**数据的逻辑结构**，它是数据结构在用户面前呈现的形式。
- 数据元素及其关系在计算机存储器中的存储方式，即**数据的存储结构**，也成为数据的物理结构。
- 施加在数据上的操作，即数据的运算。

数据的逻辑结构主要有：集合、线性结构、树形结构、图形结构。树形和图形结构统称为非线性结构。

存储结构类型主要有4种：顺序存储结构、链式存储结构、索引存储结构、散列（哈希）存储结构。

**什么是算法？**

数据元素之间的关系有逻辑关系和物理关系，对应的运算有逻辑结构上的运算(抽象运算)和具体存储结构上的运算(运算实现)。算法是在具体存储结构上实现某个抽象运算。算法是对特定问题的求解步骤的一种描述。算法有如下五个重要特性：

有穷性、确定性、可行性、有输入、有输出。

算法和程序不同，程序可以无穷循环，可以不一定满足有穷性，但算法必须满足有穷性，程序 = 数据结构 + 算法。

# 一、线性表

线性表是**具有相同特性的数据元素的一个有限序列**。线性表按照存储结构可以分为**顺序表**(顺序存储结构)、**链表**(链式存储结构)。

## 1.1.顺序表（如ArrayList）

Java中**ArrayList**集合的底层也是一种顺序表。（存储数据溢出时有自动扩容机制）

顺序表存储数据时，会提前申请一整块足够大小的物理空间，然后将数据依次存储起来，存储时做到数据元素之间不留一丝缝隙。使用顺序表存储数据之前，除了要申请足够大小的物理空间之外，为了方便后期使用表中的数据，顺序表还需要实时记录以下 2 项数据：

1. 顺序表申请的存储容量；
2. 顺序表的长度，也就是表中存储数据元素的个数；哦l

## 1.2.链表

顺序表必须占用一整块事先分配大小的固定的存储空间，不便于存储空间的管理。为此提出了可以实现存储空间动态管理的链式存储方式，即链表。

![image-20210322165146288](DataStructuresAndAlgorithms.assets/image-20210322165146288.png)
...
### 1.2.2.双链表（如LinkedList）

```java
class Node<T>{
    public T item; //节点的Date数据
    public Node pre;
    public Node next;
...
队列简称队，它是一种操作受限的线性表，其限制为仅允许在表的一端(尾部)进行插入，而在表的另一端(头部)进行删除，即先入先出，后入后出。

有序列表，可用数组与链表（linkedList中的removeFirst和addLast方法）实现。
...
栈是一种只能在一端进行插入或删除操作的线性表。先入后出，后入先出。
...
- 树相较于顺序存储和链式存储能提高数据**存储，读取**的效率
  ...
  ![image-20210525190112848](DataStructuresAndAlgorithms.assets/image-20210525190112848.png)
...
- ![image-20210728144043481](DataStructuresAndAlgorithms.assets/image-20210728144043481.png)
...
![image-20210526202744674](DataStructuresAndAlgorithms.assets/image-20210526202744674.png)
...
![image-20210526203151815](DataStructuresAndAlgorithms.assets/image-20210526203151815.png)

![image-20210526203207137](DataStructuresAndAlgorithms.assets/image-20210526203207137.png)
...
- 大顶堆举例说明：
  ...
  ![image-20210729151251530](DataStructuresAndAlgorithms.assets/image-20210729151251530.png)
...
- 创建赫夫曼树思路：
  ...
  <img src="DataStructuresAndAlgorithms.assets/image-20210729223216180.png" alt="image-20210729223216180" style="zoom:50%;" />
...
- 介绍：
  ...
  - 在二叉排序树的构建过程中，会造成对于任何一个非叶子结点，左子树的所有结点的值比当前结点的值小，右子树的所有结点的值比当前结点的值大，相同的值一般放在右子结点。
    ...
    ![image-20210802141308608](DataStructuresAndAlgorithms.assets/image-20210802141308608.png)
...
- 二叉排序树的结点删除：
  - ![image-20210802141912503](DataStructuresAndAlgorithms.assets/image-20210802141912503.png)
  - 删除叶子结点 (比如：2, 5, 9, 12)：
...
- 介绍：
  ...
  - 数列{1,2,3,4,5,6}，创建一颗二叉排序树(BST)，左子树全部为空，从形式上看，更像一个单链表，插入速度没有影响查询速度明显降低(因为需要依次比较)， 不能发挥BST的优势，因为每次还需要比较左子树，其查询速度比单链表还慢
    ...
    <img src="DataStructuresAndAlgorithms.assets/image-20210802193440057.png" alt="image-20210802193440057" style="zoom: 67%;" />
...
- **AVL树左旋转：**
  ...
  <img src="DataStructuresAndAlgorithms.assets/image-20210802194040450.png" alt="image-20210802194040450" style="zoom:67%;" />
...
<img src="DataStructuresAndAlgorithms.assets/image-20210802194427681.png" alt="image-20210802194427681" style="zoom: 80%;" />
...
![image-20210802194619951](DataStructuresAndAlgorithms.assets/image-20210802194619951.png)
...
- 多叉树：
  ...
  - <img src="DataStructuresAndAlgorithms.assets/image-20210802214332593.png" alt="image-20210802214332593" style="zoom:80%;" />
...
- 2-3树构建举例{16, 24, 12, 32, 14, 26, 34, 10, 8, 28, 38, 20} **符合二叉排序树的规则**：
  - ![image-20210802214730847](DataStructuresAndAlgorithms.assets/image-20210802214730847.png)
...
<img src="DataStructuresAndAlgorithms.assets/image-20210802214917153.png" alt="image-20210802214917153" style="zoom:80%;" />
...
- B树的说明：
  ...
  ![image-20210802215328730](DataStructuresAndAlgorithms.assets/image-20210802215328730.png)
...
- B+树的说明：
  ...
  ![image-20210802215452159](DataStructuresAndAlgorithms.assets/image-20210802215452159.png)
...
![image-20210802215615450](DataStructuresAndAlgorithms.assets/image-20210802215615450.png)
...
- 图的常用术语：
  ...
  ![image-20210803165148688](DataStructuresAndAlgorithms.assets/image-20210803165148688.png)

  ![image-20210803165214467](DataStructuresAndAlgorithms.assets/image-20210803165214467.png)
...
- 邻接矩阵
  ...
  <img src="DataStructuresAndAlgorithms.assets/image-20210803165434134.png" alt="image-20210803165434134" style="zoom: 67%;" />
...
- 邻接表
  ...
  ![image-20210803165935507](DataStructuresAndAlgorithms.assets/image-20210803165935507.png) 
...
![image-20210803170432781](DataStructuresAndAlgorithms.assets/image-20210803170432781.png)
...
![image-20210511152306229](DataStructuresAndAlgorithms.assets/image-20210511152306229.png)
...
## 6.6.归并排序MergeSorting

分治思想（divide and conquer）

![image-20210803204705136](DataStructuresAndAlgorithms.assets/image-20210803204705136.png)
...
![image-20210512125946805](DataStructuresAndAlgorithms.assets/image-20210512125946805.png)

![image-20210512125905584](DataStructuresAndAlgorithms.assets/image-20210512125905584.png)
...
![image-20210729094323286](DataStructuresAndAlgorithms.assets/image-20210729094323286.png)

![image-20210512132412367](DataStructuresAndAlgorithms.assets/image-20210512132412367.png)
...
- 汉诺塔问题分析：
  ...
  - 2)如果我们有 n >= 2 情况，我们总是可以看做是两个盘 1.最下边的盘(第n个) 2. 上面的盘(第1到n-1个)
    ...
    - 把最下边的盘 A->C
    - 把B塔的所有盘 从 B->C 
    - ![image-20210804102911540](DataStructuresAndAlgorithms.assets/image-20210804102911540.png)
...
- 01背包问题：
  ...
  - 问题描述：
    ...
    ![image-20210804145154514](DataStructuresAndAlgorithms.assets/image-20210804145154514.png)
  ...
  - 解题思路：每次遍历到的第i个物品，根据w[i]和val[i]来确定是否需要将该物品放入背包中。即对于给定的n个物品，设val[i]、w[i]分别为第i+1(从0开始)个物品的价值和体积，C为背包的容量。再令v[i] [j]表示在前 i 个物品中能够装入容量为 j 的背包中的最大价值。
    ...
    - (1)  v[i] [0]=v[0] [j]=0; //表示填入表第一行和第一列是0
      ...
      ![image-20210804183103594](DataStructuresAndAlgorithms.assets/image-20210804183103594.png)
    ...
    - (3) 当j>=w[i-1]时：// 当准备加入新增的商品的容量小于或等于当前背包的容量时
      ...
      ![image-20210804183124961](DataStructuresAndAlgorithms.assets/image-20210804183124961.png)
    ...
    - 背包问题最优解回溯（从最后一个解v[i] [j]开始）：
      ...
      ![image-20210804183744802](DataStructuresAndAlgorithms.assets/image-20210804183744802.png)
...
- 暴力匹配算法：
  ...
  - ![image-20210805145442763](DataStructuresAndAlgorithms.assets/image-20210805145442763.png)

    ![image-20210805145500145](DataStructuresAndAlgorithms.assets/image-20210805145500145-163145306271738.png)
...
- KMP算法：
  ...
  - 获取模式串的部分匹配表（**使用了KMP后续算法的原理**）：
    ...
    - ![image-20210805150109856](DataStructuresAndAlgorithms.assets/image-20210805150109856.png)
  ...
  - ![image-20210805150738117](DataStructuresAndAlgorithms.assets/image-20210805150738117-163145306271740.png)

    <img src="DataStructuresAndAlgorithms.assets/image-20210805150752631.png" alt="image-20210805150752631"  />
...
- 应用-集合覆盖：存在如下表的需要付费的广播台，以及广播台信号可以覆盖的地区。 **如何选择最少的广播台**，让所有的地区都可以接收到信号
  ...
  - <img src="DataStructuresAndAlgorithms.assets/image-20210805193444352.png" alt="image-20210805193444352" style="zoom:80%;" />
    ...
    public static void main(String[] args) {
        ...
        while (allAreas.size() != 0) {
            ...
            for (String key : broadcasts.keySet()) {
                ...
                //tempSet.size() > preMaxKeyNum体现出贪心算法的特点
                if (tempSet.size() > 0 &&
...
![image-20210807232021381](DataStructuresAndAlgorithms.assets/image-20210807232021381.png)

- 应用场景-修路问题：
  - ![image-20210807233107547](DataStructuresAndAlgorithms.assets/image-20210807233107547.png)
...
![image-20210807233014282](DataStructuresAndAlgorithms.assets/image-20210807233014282.png)
...
- 克鲁斯卡尔(Kruskal)算法，同**Prim算法一样是用来求加权连通图的最小生成树的算法**

- 应用场景-修路问题：

  - 各个站点的距离用边线表示(权) ，如何修路保证各个村庄都能连通，并且总的修建公路总里程最短?
  - <img src="DataStructuresAndAlgorithms.assets/image-20210808203743814.png" alt="image-20210808203743814" style="zoom:50%;" />

- 基本思想：按照权值从小到大的顺序选择n-1条边，并保证在选择边时不构成回路

- 图解：

  ![微信截图_20210807233707](DataStructuresAndAlgorithms.assets/微信截图_20210807233707.png)

- 保证在选择边时不构成回路：

  - 即在添加边时边的两个顶点不能指向同一个终点
  - 顶点的终点是指在**当前最小生成树中与其连通的最大顶点**，将顶点数组映射到【0,1,2...数组】，**在添加边时要存储ends[start]=end**（start>end），以便查找顶点的终点
  - ![image-20210807234900478](DataStructuresAndAlgorithms.assets/image-20210807234900478.png)
...
- 应用场景-最短路径问题：
  - ![image-20210808154049427](DataStructuresAndAlgorithms.assets/image-20210808154049427.png)
...
![无标题](DataStructuresAndAlgorithms.assets/无标题.png)
...
- 应用场景-最短路径问题：
  - ![image-20210808204020088](DataStructuresAndAlgorithms.assets/image-20210808204020088.png)
...
- 算法思路和图解：
  ...
  - ![image-20210808204701749](DataStructuresAndAlgorithms.assets/image-20210808204701749.png)
  - 把A作为中间顶点的所有情况都进行遍历后得到
  - ![image-20210808204754758](DataStructuresAndAlgorithms.assets/image-20210808204754758.png)
...
![image-20210808232049122](DataStructuresAndAlgorithms.assets/image-20210808232049122.png)
